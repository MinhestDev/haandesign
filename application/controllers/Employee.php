...
class Employee extends CI_Controller {
	...
	$blood = $this->input->post('blood');		
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if($this->employee_model->Does_email_exists($email) && $password != $confirm){
                ...
                echo "Email đã tồn tại hoặc Kiểm tra mật khẩu của bạn";
            } else {
	...
	$blood = $this->input->post('blood');		
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $checkimage = "./assets/images/users/$employee->em_image";                 
                ...
                if($id){
            $success = $this->employee_model->Update($data,$id); 
            $this->session->set_flashdata('feedback','Đã cập nhật thành công!');
            echo "Cập nhật thành công";
                }
			}
        } else {
    ...
    public function Parmanent_Address(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(!empty($id)){
                ...
                $this->session->set_flashdata('feedback','Đã thêm thành công!');
                echo "Thêm Thành Công";
            }
    ...
    public function Reset_Password_Hr(){
        ...
        $this->session->set_flashdata('feedback', 'Vui lòng nhập mật khẩu hợp lệ');
        #redirect("employee/view?I=" .base64_encode($id)); 
    ...
    public function Reset_Password(){
        ...
        $this->session->set_flashdata('feedback', 'Vui lòng nhập mật khẩu hợp lệ');
        #redirect("employee/view?I=" .base64_encode($id));
                echo "Vui lòng nhập mật khẩu hợp lệ";
            }
        } else {
            $this->session->set_flashdata('feedback', 'Vui lòng nhập mật khẩu hợp lệ');
            #redirect("employee/view?I=" .base64_encode($id));
    ...
    public function Add_Salary(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(!empty($sid)){
                ...
                #$this->session->set_flashdata('feedback','Cập nhật thành công');
                #echo "Cập nhật thành công";
	...
	public function confirm_mail_send($email,$pass_hash){
		...
		'smtp_pass' => ''
         ...
         $this->email->subject('Hệ thống Hà An'); 
		 $message	 =	"Email đăng nhập của bạn là:"."$email";
		 $message	.=	"Mật khẩu của bạn :"."$pass_hash"; 
         $this->email->message($message); 
   
         //Send mail 
         if($this->email->send()){ 
         	$this->session->set_flashdata('feedback', 'Vui lòng kiểm tra email của bạn Để đặt lại mật khẩu của bạn');
		 }
         else {
         $this->session->set_flashdata("feedback", "Lỗi gửi Email."); 
		 }			
	}
